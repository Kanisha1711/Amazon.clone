(() => {
  ...
  var Cart = class {
    ...
    loadItems() {
      try {
        ...
        const normalizedItems = parsedItems.filter((item) => item && item.productId).map((item) => ({
  ...
  // data/products.js
  var Product = class {
    constructor({ id, image, name, rating, priceCents, keywords, type, sizeChartLink }) {
      Object.assign(this, { id, image, name, rating, priceCents, keywords, type, sizeChartLink });
    }
  };
  var productData = [
    {
  ...
  var products = productData.map((product) => new Product(product));

  // scripts/utils/money.js
  var MoneyFormatter = class {
    format(priceCents) {
  ...
  var deliveryOptions = deliveryOptionData.map((option) => new DeliveryOption(option));
  ...
  var CheckoutPage = class {
    constructor({ cart: cart2, products: products2, deliveryOptions: deliveryOptions2, document: document2 }) {
      this.cart = cart2;
      this.products = products2;
      this.deliveryOptions = deliveryOptions2;
    ...
    renderSummary() {
      ...
      summary.innerHTML = this.cart.length ? this.cart.map((cartItem) => this.renderCartItem(cartItem)).join("") : '<div class="empty-cart">Your cart is empty.</div>';
    }
    ...
    bindEvents() {
      ...
      this.document.querySelectorAll(".js-delivery-option").forEach((optionElement) => {
        optionElement.addEventListener("click", () => {
          ...
          updateDeliveryOption(productId, deliveryOptionId);
          const date = formatDeliveryDate(option.deliveryDays);
          this.document.querySelector(`.js-cart-item-container-${productId} .delivery-date`).innerHTML = `Delivery date: ${date}`;
          this.updatePaymentSummary();
        });
      });
